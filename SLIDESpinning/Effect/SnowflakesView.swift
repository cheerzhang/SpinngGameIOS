...
public class SnowflakesView : UIView {
  ...
  override public init (frame : CGRect) {
    ...
    initGravityAnimator()
    timer = Timer.scheduledTimer(timeInterval: Double(arc4random_uniform(100))/100, target: self, selector: #selector(self.changeGravityDirection), userInfo: nil, repeats: true)
  }
  ...
  func addNewSnowflake() {
    ...
    let snowflake = Snowflake(position: snowflakePosition, minimumSize: 8, maximumSize: 10)
